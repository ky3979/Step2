...
/// Fish filename
const wstring FishStarfishImageName = L"images/starfish.png";
...
*/
CFishStarfish::CFishStarfish(CAquarium* aquarium) : CItem(aquarium)
{
	// Load image from disk
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishStarfishImageName.c_str()));

	// Error check
	if (mFishImage->GetLastStatus() != Ok)
	{
		wstring msg(L"Failed to open");
		msg += FishStarfishImageName;
...
* \return true if hit
*/
bool CFishStarfish::HitTest(int x, int y)
{
...
*/
void CFishStarfish::Draw(Gdiplus::Graphics* graphics)
{
	double width = mFishImage->GetWidth();
	double height = mFishImage->GetHeight();
